...
NSArray<NSDictionary *> *LGRendererItemsForHostPrefix(NSString *prefix) {
    ...
    return @[
        ...
        LGSliderSetting(key(@"CenterTintFactor"),
                        ...
                        centerTintDefault, 0.0, 1.0, 2),
        LGSliderSetting(key(@"CenterTintFactorDark"),
                        LGLocalized(@"prefs.control.center_tint_factor_dark"),
                        LGLocalized(@"prefs.subtitle.center_tint_factor"),
                        centerTintDefault, 0.0, 1.0, 2),
        @{
...
NSArray<NSDictionary *> *LGDynamicIsland2Items(void) {
    return LGJoinItemGroups(@[
        ...
        @[
            LGSectionSetting(@"灵动岛2", @"开启后将使用原版 Liquid (Gl)ass 灵动岛液态效果，原版灵动岛自动关闭"),
            LGGlassEnabledSetting(@"DynamicIsland2.Enabled", NO),
        ],
        // DI2 玻璃渲染参数（使用 Island 滤镜，原版参数）
        LGRendererItemsForHostPrefix(@"Island"),
        // DI2 功能开关
        @[
            LGSectionSetting(@"功能设置", nil),
            LGSwitchSetting(@"DynamicIsland2.HideWhenInactive",
                            @"无活动时隐藏",
                            @"系统灵动岛处于空闲状态时自动隐藏液态效果",
                            NO),
        ],
    ]);
}
